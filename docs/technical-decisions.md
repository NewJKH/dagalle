...
1순위  구글 DirectionsService  → 실제 도로·노선 (성공 시 거리·환승정보까지 사용)
  ↓  OK 아님
2순위  OSRM (오픈소스 경로 엔진) → 도보·차량 실제 도로 경로
  ↓  실패
...
- **도보·차량**: 구글이 실패하면 OSRM에 다시 요청해 실제 도로 경로를 받아왔다. OSRM이
  주는 인코딩 경로를 풀기 위해 polyline 디코더를 직접 구현했다.
- **지하철·기차**: 대체 노선 데이터를 구하기 어려워, 두 점을 직선으로 잇고 소요시간은
  AI가 만든 일정값(`durationMinutes`)으로 표시했다.
- **경쟁 상태 방지**: Day를 빠르게 전환하면 이전 요청의 응답이 늦게 도착해 엉뚱한 선이
  남는 문제가 있어, `AbortController`로 Day 전환 시 진행 중 요청을 취소했다.
